"""
Метод .insert() при попытке вставить значение с существующим ключом, вызывала исключение ValueError.
Перепишите метод так, чтобы при вставке элемента с существующим ключом, значение менялось на новое.
"""
...
class HashTable:
    ...
    def insert(self, key, value):
        ...
        # Ищем значение в хэш-таблице, если оно уже есть, то меняем на новое.
        cell = self.get(key)
        if cell is not None:
            cell.value = value
            return
